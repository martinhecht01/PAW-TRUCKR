...
public class UserServiceImplTest {
    ...
    public void testCreateUser() {
        ...
        when(userDao.create(eq(EMAIL), eq(NAME), eq(CUIT), eq(ROLE), eq(PASSWORD)))
                .thenReturn( new User(USERID, EMAIL, NAME, CUIT, ROLE, PASSWORD, false, null));

        when(passwordEncoder.encode(anyString())).thenReturn(PASSWORD);

        when(userDao.getUserById(eq(USERID)))
                .thenReturn(Optional.of(new User(USERID, EMAIL, NAME, CUIT, ROLE, PASSWORD, false, null)));


        //2 Ejercitar
        User user = userService.createUser(EMAIL, NAME, CUIT, ROLE, PASSWORD);
    ...
    public void testCreateUserWithNullId() {
        //2 Ejercitar
        User user = userService.createUser(EMAIL, NAME, null, ROLE, PASSWORD);
