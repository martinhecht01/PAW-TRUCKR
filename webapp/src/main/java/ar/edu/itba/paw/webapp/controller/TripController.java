...
public class TripController {
    ...
    public ModelAndView tripDetail(@RequestParam("id") int id, @ModelAttribute("acceptForm") final AcceptForm form) {
        ...
        if (user != null){
            ...
            mav.addObject("reviewed", revs.getReviewByTripAndUserId(id, ts.getTripById(id).orElseThrow(TripNotFoundException::new).getUserId()).orElse(null)); //TODO: fijarse si existe una review para este trip de este usuario
    ...
    public ModelAndView manageTrip(@RequestParam("tripId") int tripId, @ModelAttribute("acceptForm") final AcceptForm form ) {
        ...
        if(trip.getAcceptUserId() > 0) {
            ...
            mav.addObject("reviewed", revs.getReviewByTripAndUserId(tripId, trip.getAcceptUserId()).orElse(null)); //TODO: fijarse si existe una review para este trip de este usuario
        }
            mav.addObject("trip", trip);
